...
def train_multi_input_model(model, criterion, optimizer, train_loader, val_loader, n_epochs, metric):
    history = {"train_losses": [],
               "train_metrics": [], "validation_metrics": []}
    ...
    for epoch in range(n_epochs):
        ...
        history["train_metrics"].append(metric.compute().item())
        history["validation_metrics"].append(
            evaluate_multi_input_model(model, val_loader, metric).item())
...
def evaluate_multi_input_model(model, data_loader, metric):
    model.eval()
    metric.reset()
    with torch.no_grad():
